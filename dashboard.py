...
@st.cache_data
def load_data():
    hfgi_df = pd.read_parquet(config.DATA_DIR / "hfgi.parquet")
    primary_path = config.DATA_DIR / f"{config.sanitize_ticker(config.PRIMARY_TICKER)}.parquet"
    price_df = pd.read_parquet(primary_path)
    return hfgi_df, price_df


def main() -> None:
    st.title("HFGI Pro — Hynix Fear & Greed Index Dashboard")

    try:
        hfgi_df, price_df = load_data()
    except FileNotFoundError:
        st.error("找不到資料,請先在終端機執行 `python run.py` 下載資料並計算指標。")
        return

    engine = HFGIEngine()
    ind = engine.compute_indicators({config.PRIMARY_TICKER: price_df})
    result = run_backtest(hfgi_df)

    latest = hfgi_df.iloc[-1]
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("最新 HFGI", f"{latest['HFGI']:.1f}", latest["State"])
    col2.metric("收盤價", f"{latest['Close']:.2f}")
    col3.metric("RSI(14)", f"{ind['RSI'].iloc[-1]:.1f}")
    col4.metric("ADR Premium (proxy)", f"{latest['ADR_Premium_Raw'] * 100:.2f}%")

    summary = result.summary()
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("CAGR", f"{summary['cagr'] * 100:.2f}%")
    s2.metric("Sharpe Ratio", f"{summary['sharpe_ratio']:.2f}")
    s3.metric("Max Drawdown", f"{summary['max_drawdown'] * 100:.2f}%")
    ...
    fig_hfgi.update_layout(title="HFGI 曲線", yaxis_range=[0, 100], height=350)
    ...
    fig_price.update_layout(title="股價 & Buy/Sell 訊號", height=400)
    st.plotly_chart(fig_price, use_container_width=True)

    # --- RSI / MACD / Volume / ADR Premium in a 2x2 grid -------------------
    fig = make_subplots(rows=2, cols=2, subplot_titles=("RSI(14)", "MACD", "成交量", "ADR Premium (proxy)"))
